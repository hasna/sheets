...
function validateCell(value: unknown, a1: string): Cell {
...
function validateSheet(value: unknown, index: number): Sheet {
  ...
  if (rawCells && typeof rawCells === "object") {
    for (const [a1, cell] of Object.entries(rawCells as Record<string, unknown>)) {
      cells[a1.toUpperCase()] = validateCell(cell, a1);
    }
  }
...
export function validateWorkbook(value: unknown): Workbook {
  ...
  const sheets = rawSheets.map((s, i) => validateSheet(s, i));
...
export function deserializeWorkbook(input: string | WorkbookDocument | Workbook): Workbook {
  ...
  const candidate =
    raw && typeof raw === "object" && "workbook" in raw
      ...
      : raw;
  return recalc(validateWorkbook(candidate));
}

/** Load a workbook from a serialized document or bare workbook (SDK entry point). */
export function loadWorkbook(input: string | WorkbookDocument | Workbook): Workbook {
  return deserializeWorkbook(input);
}
